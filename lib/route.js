...
const { isUnique, prettyIds, handleError, unPrettyIds, } = require('./common');
const { validateInputItems } = require('./lifecycle.js');
module.exports = ({ modelName, schema, handlers: { beforeAddQuery, beforePatchQuery, beforeDeleteQuery, afterGetQuery, afterAddQuery } = {} }) => (app, db) => {
    ...
    app.post(`/${modelName}`, (req, res) => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            items = yield validateInputItems(schema, items);
            if (!(yield isUnique(db, modelName, schema, items)))
    ...
    app.patch(`/${modelName}`, (req, res) => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!(items instanceof Array) || items.length === 0)
                throw new Error(`Invalid request: ${modelName} must be an non-empty array.`);
            items = yield validateInputItems(schema, unPrettyIds(items));
            if (!(yield isUnique(db, modelName, schema, items)))
                throw new Error('Elements not unique');
            items = items.map(item => _.omitBy(item, i => i === undefined));
